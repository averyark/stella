return function(Iris, widgets)
	...
	local function UpdateChildContainerTransform(thisWidget)
		...
		local ChildContainerHeight = (
			thisWidget.LabelHeight * math.min(thisWidget.NumChildrenForSize, 8) - 2 * ChildContainerBorderSize
		) + (3 * Iris._config.FramePadding.Y)
		...
		if PreviewLabel.AbsolutePosition.Y + thisWidget.LabelHeight + ChildContainerHeight > ScreenSize.Y then
			...
			ChildContainer.Position = UDim2.new(
				0,
				PreviewLabel.AbsolutePosition.X + ChildContainerBorderSize,
				0,
				PreviewLabel.AbsolutePosition.Y - ChildContainerBorderSize - ChildContainerHeight
			)
		else
			ChildContainer.Position = UDim2.new(
				0,
				PreviewLabel.AbsolutePosition.X + ChildContainerBorderSize,
				0,
				PreviewLabel.AbsolutePosition.Y + thisWidget.LabelHeight + ChildContainerBorderSize
			)
		end
	end
