return function(Iris, widgets)
	local function relocateTooltips()
		...
		local newPosition = widgets.findBestWindowPosForPopup(
			...
			Vector2.new(Iris._config.DisplaySafeAreaPadding, Iris._config.DisplaySafeAreaPadding),
	...
	Iris.WidgetConstructor("Window", {
		...
		Generate = function(thisWidget)
			...
			WindowButton.InputBegan:Connect(function(input)
				...
				if not thisWidget.arguments.NoMove and input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					isDragging = true
					moveDeltaCursorPosition = widgets.UserInputService:GetMouseLocation()
						- thisWidget.state.position.value
				end
			end)
			...
			local ChildContainerUIListLayout = widgets.UIListLayout(
				ChildContainer,
				Enum.FillDirection.Vertical,
				UDim.new(0, Iris._config.ItemSpacing.Y)
			)
			...
			ResizeBorder.Size =
				UDim2.new(1, Iris._config.WindowResizePadding.X * 2, 1, Iris._config.WindowResizePadding.Y * 2)
			ResizeBorder.Position =
				UDim2.fromOffset(-Iris._config.WindowResizePadding.X, -Iris._config.WindowResizePadding.Y)
			WindowButton.AutomaticSize = Enum.AutomaticSize.None
		...
		Update = function(thisWidget)
			...
			local TitleButtonPaddingSize = Iris._config.FramePadding.X
				+ Iris._config.TextSize
				+ Iris._config.FramePadding.X * 2
		...
		GenerateState = function(thisWidget)
			...
			if thisWidget.state.position == nil then
				thisWidget.state.position = Iris._widgetState(
					...
					"position",
					if anyFocusedWindow
						then focusedWindow.state.position.value + Vector2.new(15, 45)
						else Vector2.new(150, 250)
				)
			end
