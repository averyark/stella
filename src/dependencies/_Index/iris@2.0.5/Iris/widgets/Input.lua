...
return function(Iris, widgets)
	...
	local abstractInputVector3 = {
		...
		GenerateState = function(thisWidget)
			if thisWidget.state.number == nil then
				...
				thisWidget.state.number = Iris._widgetState(
					thisWidget,
					"number",
					Vector3.new(math.clamp(0, Min.X, Max.X), math.clamp(0, Min.Y, Max.Y), math.clamp(0, Min.Z, Max.Z))
				)
			end
		end,
		UpdateState = function(thisWidget)
			...
			local newTextX = string.format(
				thisWidget.arguments.Format
					or (
						(thisWidget.arguments.Increment and thisWidget.arguments.Increment.X or 1) >= 1 and "%d" or "%f"
					),
				thisWidget.state.number.value.X
			)
			local newTextY = string.format(
				thisWidget.arguments.Format
					or (
						(thisWidget.arguments.Increment and thisWidget.arguments.Increment.Y or 1) >= 1 and "%d" or "%f"
					),
				thisWidget.state.number.value.Y
			)
			local newTextZ = string.format(
				thisWidget.arguments.Format
					or (
						(thisWidget.arguments.Increment and thisWidget.arguments.Increment.Z or 1) >= 1 and "%d" or "%f"
					),
				thisWidget.state.number.value.Z
			)
		...
		GenerateState = function(thisWidget)
			if thisWidget.state.number == nil then
				...
				thisWidget.state.number = Iris._widgetState(
					thisWidget,
					"number",
					Vector2.new(math.clamp(0, Min.X, Max.X), math.clamp(0, Min.Y, Max.Y))
				)
			end
		end,
		UpdateState = function(thisWidget)
		...
		GenerateState = function(thisWidget)
			if thisWidget.state.number == nil then
				...
				thisWidget.state.number = Iris._widgetState(
					thisWidget,
					"number",
					UDim.new(math.clamp(0, Min.Scale, Max.Scale), math.clamp(0, Min.Offset, Max.Offset))
				)
			end
		end,
		UpdateState = function(thisWidget)
			...
			local formatTextOffset = thisWidget.arguments.Format
				or (
					(thisWidget.arguments.Increment and thisWidget.arguments.Increment.Offset or 1) >= 1 and "%d" or "%f"
				)
	...
	local abstractInputUDim2 = widgets.extend(abstractInputVector2, {
		...
		UpdateState = function(thisWidget)
			...
			local newTextXOffset =
				string.format("X Offset: " .. formatTextOffset, thisWidget.state.number.value.X.Offset)
			...
			local newTextYOffset =
				string.format("Y Offset: " .. formatTextOffset, thisWidget.state.number.value.Y.Offset)
		...
		Iris.WidgetConstructor("DragNum", {
			...
			Generate = function(thisWidget)
				...
				InputField.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						newValue = math.clamp(
							newValue,
							thisWidget.arguments.Min or -math.huge,
							thisWidget.arguments.Max or math.huge
						)
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment)
			...
			UpdateState = function(thisWidget)
				...
				local newText = string.format(
					thisWidget.arguments.Format or ((thisWidget.arguments.Increment or 1) >= 1 and "%d" or "%f"),
					thisWidget.state.number.value
				)
	...
	do -- Iris.SliderNum
		...
		local function updateActiveSlider()
			...
			local PositionRatio = (
				widgets.UserInputService:GetMouseLocation().X - (InputFieldContainer.AbsolutePosition.X + GrabPadding)
			) / (InputFieldContainer.AbsoluteSize.X - 2 * GrabPadding)

			local NewNumber =
				math.clamp(math.floor(PositionRatio * GrabNumPossiblePositions) * Increment + Min, Min, Max)
		...
		Iris.WidgetConstructor("SliderNum", {
			...
			Update = function(thisWidget)
				...
				local grabScaleSize = math.max(
					1 / math.floor((1 + Max - Min) / Increment),
					Iris._config.GrabMinSize / InputFieldContainer.AbsoluteSize.X
				)
			...
			UpdateState = function(thisWidget)
				...
				local MaxScaleSize = 1
					- (GrabBar.AbsoluteSize.X / (InputFieldContainer.AbsoluteSize.X - 2 * GrabPadding))
				local GrabBarPos = math.clamp(
					math.floor(PositionRatio * GrabNumPossiblePositions) / GrabNumPossiblePositions,
					0,
					MaxScaleSize
				)
	...
	Iris.WidgetConstructor("InputNum", {
		...
		Generate = function(thisWidget)
			...
			SubButton.MouseButton1Click:Connect(function()
				...
				newValue =
					math.clamp(newValue, thisWidget.arguments.Min or -math.huge, thisWidget.arguments.Max or math.huge)
				thisWidget.state.number:set(newValue)
				thisWidget.lastNumchangeTick = Iris._cycleTick + 1
			end)
		...
		Update = function(thisWidget)
			...
			local inputButtonsTotalWidth = Iris._config.TextSize * 2
				+ Iris._config.ItemInnerSpacing.X * 2
				+ Iris._config.WindowPadding.X
				+ 4
	...
	Iris.WidgetConstructor(
		"InputVector2",
		widgets.extend(abstractInputVector2, {
			Generate = function(thisWidget)
				...
				InputFieldX.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.X)
								* thisWidget.arguments.Increment.X
						end
				...
				InputFieldY.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.Y)
								* thisWidget.arguments.Increment.Y
						end
	...
	Iris.WidgetConstructor(
		"InputVector3",
		widgets.extend(abstractInputVector3, {
			Generate = function(thisWidget)
				...
				InputFieldX.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						thisWidget.state.number:set(
							Vector3.new(newValue, thisWidget.state.number.value.Y, thisWidget.state.number.value.Z)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
						InputFieldX.Text = thisWidget.state.number.value.X
				...
				InputFieldY.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						thisWidget.state.number:set(
							Vector3.new(thisWidget.state.number.value.X, newValue, thisWidget.state.number.value.Z)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
						InputFieldY.Text = thisWidget.state.number.value.Y
				...
				InputFieldZ.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.Z)
								* thisWidget.arguments.Increment.Z
						end
						thisWidget.state.number:set(
							Vector3.new(thisWidget.state.number.value.X, thisWidget.state.number.value.Y, newValue)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
	...
	Iris.WidgetConstructor(
		"InputUDim",
		widgets.extend(abstractInputUDim, {
			Generate = function(thisWidget)
				...
				InputFieldScale.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.Scale)
								* thisWidget.arguments.Increment.Scale
						end
				...
				InputFieldOffset.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.Offset)
								* thisWidget.arguments.Increment.Offset
						end
	...
	Iris.WidgetConstructor(
		"InputUDim2",
		widgets.extend(abstractInputUDim2, {
			Generate = function(thisWidget)
				...
				InputFieldXScale.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.X.Scale)
								* thisWidget.arguments.Increment.X.Scale
						end
						thisWidget.state.number:set(
							UDim2.new(
								UDim.new(newValue, thisWidget.state.number.value.X.Offset),
								thisWidget.state.number.value.Y
							)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
				...
				InputFieldXOffset.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.X.Offset)
								* thisWidget.arguments.Increment.X.Offset
						end
						thisWidget.state.number:set(
							UDim2.new(
								UDim.new(thisWidget.state.number.value.X.Scale, newValue),
								thisWidget.state.number.value.Y
							)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
				...
				InputFieldYScale.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.Y.Scale)
								* thisWidget.arguments.Increment.Y.Scale
						end
						thisWidget.state.number:set(
							UDim2.new(
								thisWidget.state.number.value.X,
								UDim.new(newValue, thisWidget.state.number.value.Y.Offset)
							)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
				...
				InputFieldYOffset.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.Increment then
							newValue = math.floor(newValue / thisWidget.arguments.Increment.Y.Offset)
								* thisWidget.arguments.Increment.Y.Offset
						end
						thisWidget.state.number:set(
							UDim2.new(
								thisWidget.state.number.value.X,
								UDim.new(thisWidget.state.number.value.Y.Scale, newValue)
							)
						)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
	...
	Iris.WidgetConstructor(
		"InputColor3",
		widgets.extend(abstractInputColor3, {
			Generate = function(thisWidget)
				...
				InputFieldR.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						newValue = math.clamp(newValue, 0, thisWidget.arguments.UseFloats and 1 or 255)
						...
						if thisWidget.arguments.UseHSV then
							...
							newValueColor =
								Color3.new(newValue, thisWidget.state.color.value.G, thisWidget.state.color.value.B)
						end
				...
				InputFieldG.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.UseHSV then
							...
							newValueColor =
								Color3.new(thisWidget.state.color.value.R, newValue, thisWidget.state.color.value.B)
						end
						thisWidget.state.color:set(newValueColor)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
				...
				InputFieldB.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						if thisWidget.arguments.UseHSV then
							...
							newValueColor =
								Color3.new(thisWidget.state.color.value.R, thisWidget.state.color.value.G, newValue)
						end
						thisWidget.state.color:set(newValueColor)
						thisWidget.lastNumchangeTick = Iris._cycleTick + 1
					else
	...
	Iris.WidgetConstructor(
		"InputColor4",
		widgets.extend(abstractInputColor4, {
			Generate = function(thisWidget)
				local InputColor = GenerateRootFrame(thisWidget, "Iris_InputColor3")
