return function(Iris)
	...
	local widgetDemos = {
		Basic = function()
			...
			helpMarker(
				"The Widgets shown are, in order, Iris.Button, Iris.SmallButton, Iris.Text, Iris.TextWrapped, Iris.TextColored, and Iris.RadioButton"
			)
			Iris.End()
		...
		Combo = function()
			...
			local InputEnum = Iris.InputEnum(
				{ "Using InputEnum" },
	...
	local function recursiveWindow(parentCheckboxState)
		Iris.Window(
			{ "Recursive Window" },
			{ size = Iris.State(Vector2.new(150, 100)), isOpened = parentCheckboxState }
		)
	...
	local function runtimeInfo()
		...
		Iris.SameLine()
		Iris.InputNum(
			{ "", [Iris.Args.InputNum.Format] = "%d Seconds", [Iris.Args.InputNum.Max] = 10 },
			...
			local styleList = {
				{
					...
					function()
						...
						for _, vVector2 in Vector2s do
							local Input = Iris.InputVector2(
								{ vVector2 },
								{ number = Iris.WeakState(Iris._config[vVector2]) }
							)
							if Input.numberChanged() then
						...
						for _, vEnum in Enums do
							local Input = Iris.InputEnum(
								{ vEnum },
								{ index = Iris.WeakState(Iris._config[vEnum]) },
								Iris._config[vEnum].EnumType
							)
